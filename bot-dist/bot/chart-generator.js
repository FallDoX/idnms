...
export async function generateChartBuffer(data, type) {
...
function drawCustomLegend(ctx, type) {
    const legendItems = [];
    if (type === 'main') {
        legendItems.push({ label: 'Speed (km/h)', color: '#3b82f6' }, { label: 'Power (W)', color: '#f59e0b' }, { label: 'Current (A)', color: '#ef4444' });
    }
    else if (type === 'acceleration') {
        legendItems.push({ label: 'Speed (km/h)', color: '#3b82f6' }, { label: 'Acceleration (m/s²)', color: '#8b5cf6' });
    }
    else {
        legendItems.push({ label: 'GPS Speed (km/h)', color: '#10b981' });
    }
...
export async function generateSummaryImage(data) {
    ...
    const maxPower = Math.max(...data.map((d) => d.Power || 0));
    const maxCurrent = Math.max(...data.map((d) => d.Current || 0));
    ctx.font = '18px Arial';
        ...
        return {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: 'Speed (km/h)',
                        data: data.map((d) => d.Speed).slice(1),
                        borderColor: '#3b82f6',
                        backgroundColor: 'rgba(59, 130, 246, 0.1)',
                        fill: true,
                        tension: 0.4,
